...
<html>
<head>
<title>3. 시장출하 적부 판정 기록 기간별 조회 [ 제조사]</title>
...
<% if Supplier="" then %>
          제조사로 <b><font color=red>[]</font></b>을(를) 검색한 결과
<% elseif Supplier <> "" then %>          
          <b>제조사로  <b><font color=red>[ <%=Supplier%> ]</font></b>을(를) 검색한 결과</span>
...
<table style="border-collapse:collapse;" align="center" cellspacing="0" width="1080">
   <tr>
       ...
       <a href="03_Maker_ok_Print.asp?Styear=<%=Styear%>&Stmonth=<%=Stmonth%>&Stday=<%=Stday%>&Fyear=<%=Fyear%>&Fmonth=<%=Fmonth%>&Fday=<%=Fday%>&Supplier=<%=Supplier%>">
       <img src="../images/print.gif" border=0></a></td>
       <td width="260"  align=left>
         <a href="03_Maker_ok(EXCEL).asp?Styear=<%=Styear%>&Stmonth=<%=Stmonth%>&Stday=<%=Stday%>&Fyear=<%=Fyear%>&Fmonth=<%=Fmonth%>&Fday=<%=Fday%>&Supplier=<%=Supplier%>">
